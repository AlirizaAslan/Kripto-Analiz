<script lang="ts">
	...
	function marketLabel(value: string) {
		if (value === 'crypto') return 'Kripto';
		if (value === 'bist') return 'BIST';
		return 'ABD Hisse';
	}
	...
	function insightSummary(
		...
		return items
			.map((entry) => {
				...
				return `${hourLabelTR(entry.hour)} (${formatPercent(rate)}, n=${entry.sampleSize})`;
			})
			.join(', ');
	}
...
<svelte:head>
	<title>{item.symbol} Istatistikleri | PulseAlpha</title>
</svelte:head>

<div class="shell">
	<header class="masthead">
		<div class="brand">
			<p class="eyebrow">{marketLabel(item.market)} / Kalici performans kayitlari</p>
			<h1>{item.symbol} icin ayrintili istatistikler</h1>
			<p class="intro">
				Kalici veritabanindaki tahmin gecmisinden uretilen saatlik dogruluk, hacim ve hata davranisini bu sayfada izleyin.
			</p>
		</div>
		<div class="controls">
			<p class="eyebrow">Son hedef mum</p>
			<strong>{formatDateTime(item.lastTargetCandleStart)}</strong>
			<a href={`/markets/${detail.asset.market}/${detail.asset.symbol}`} class="back-link">
				Canli sembol ekranina don
			</a>
			<a href="/statistics" class="back-link subtle">Tum istatistiklere don</a>
		</div>
	</header>

	<section class="hero-grid">
		<article class="hero-card">
			<p class="eyebrow">Sembol ozeti</p>
			<h2>{detail.asset.name}</h2>
			<p class="meta-line">
				<span>{detail.asset.venue}</span>
				<span>{detail.asset.sessionLabel}</span>
				<span>Son fiyat {formatPrice(detail.asset.lastPrice)}</span>
			</p>
			<p class="summary">{detail.asset.thesis}</p>
		</article>

		<article class="hero-card">
			<p class="eyebrow">Veri kaliciligi</p>
			<h2>SQLite tabanli istatistikler</h2>
			<p class="summary">
				Bu sayfadaki tum performans metrikleri kapanan tahminlerin kalici veritabani kayitlarindan hesaplanir; tarayici kapaninca sifirlanmaz.
			</p>
		</article>
	</section>
...
<style>
	...
	.hero-grid, .kpi-grid, .insight-grid, .detail-grid {
		display: grid;
		gap: 16px;
	}
	...
	.hero-card, .kpi-grid div, .insight-card, .detail-card {
		padding: 20px;
		border-radius: 22px;
	...
	.intro, .summary, .meta-line, .section-note, .status-block p {
		color: #5f6e81;
		line-height: 1.6;
	}
